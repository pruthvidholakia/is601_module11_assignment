...
def test_session_handling(db_session):
    ...
    """
    initial_user_total = db_session.query(User).count()
    logger.info(f"Initial user count before test_session_handling: {initial_user_total}")
    assert initial_user_total == 0, f"Expected 0 users before test, found {initial_user_total}"
    ...
    db_session.commit()
    logger.info(f"Added user1: {first_user.email}")

    after_user1_count = db_session.query(User).count()
    logger.info(f"User count after adding user1: {after_user1_count}")
    assert after_user1_count == 1, f"Expected 1 user after adding user1, found {after_user1_count}"

    # Attempt to commit user2 (duplicate email)
    try:
    ...
    existing_user = db_session.query(User).filter_by(email="test1@example.com").first()
    assert existing_user is not None, "User1 should still exist after rollback"
    assert existing_user.username == "testuser1"
    logger.info(f"Found user1 after rollback: {existing_user.email}")

    # Add and commit user3
    third_user = User(
        first_name="Test",
        last_name="User",
    ...
    db_session.commit()
    logger.info(f"Added user3: {third_user.email}")

    # Final check
    all_users = db_session.query(User).order_by(User.email).all()
    final_user_count = len(all_users)
    all_emails = {u.email for u in all_users}
    logger.info(f"Final user count: {final_user_count}, Emails: {all_emails}")

    assert final_user_count == 2, f"Should have exactly user1 and user3, found {final_user_count}"
    assert "test1@example.com" in all_emails, "User1 must remain"
    assert "test3@example.com" in all_emails, "User3 must exist"
